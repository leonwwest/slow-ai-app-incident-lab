...
async def db_query(request: Request) -> dict:
    ...
    Runs a real SQLite recursive-CTE scan and adds a random artificial delay
    so latency is measurable and reproducible under load. db_latency_ms is
    recorded on request.state so it shows up in logs and the DB.
    """
    delay_ms = random.randint(300, 1500)
    start = time.perf_counter()
    time.sleep(delay_ms / 1000.0)

    # A deliberately un-indexed scan-ish query: generate rows and aggregate.
